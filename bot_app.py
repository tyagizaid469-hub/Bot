            ...
            for m in msgs:
                ...
                if not found_handles:
                    try:
                        ...
                        print(f"[SYNC] body fetch error for {mid}: {body_err!r}")
...
async def _otp_sync_loop(svc, poll_sec: int = 10):
    ...
    print("[OTP-SYNC] OTP sync loop started")
    ...
    while True:
        try:
                    ...
                    for gmail_handle in all_handles:
                        ...
                        if user_id and _otp_app_ref:
                            try:
                                ...
                                print(f"[OTP-SYNC] Delivered OTP {otp} → user {user_id} ({gmail_handle})")
                                break
                            except Exception as e:
                                print(f"[OTP-SYNC] Delivery failed user {user_id}: {e!r}")
            ...
            print(f"[OTP-SYNC] tick error: {e!r}")
...
async def menu_handler(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if txt_is(txt, "menu_accounts"):
        ...
        cur.execute("""
            ...
            WHERE a.user_id=? AND a.state IN (
    ...
    try:
        if os.path.exists("token.json") or os.environ.get("GMAIL_TOKEN_JSON"):
            ...
            try:
                ...
                print(f"[OTP-SYNC] scheduled otp sync task (poll_sec={otp_poll_sec})")
            except Exception as oe:
                print(f"[OTP-SYNC] failed to start: {oe!r}")
        else:
