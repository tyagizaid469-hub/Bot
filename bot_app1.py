...
def db():
    # Use a safer SQLite configuration for concurrent access (Railway/webhook async).
    con = sqlite3.connect(DB, timeout=1, check_same_thread=False)
...
def init_db():
    ...
    for col in [
        ...
        "ALTER TABLE registrations ADD COLUMN status TEXT DEFAULT 'created'",
        "ALTER TABLE form_table ADD COLUMN msg_id INTEGER"
    ]:
        try:
            cur.execute(col)
        except Exception:
    ...
    # Email check logs (used for stats; safe if unused)
    cur.execute("""
...
def reg_buttons(action_id, task_id=None):
    return InlineKeyboardMarkup([
        [InlineKeyboardButton("DONE✅", callback_data=f"REG_DONE:{action_id}:{task_id}")],
        [InlineKeyboardButton("CANCEL ❌ REGISTRATION", callback_data=f"REG_CANCEL:{action_id}:{task_id}")],
...
def _queue_userbot_job(job_type: str, user_id: int, payload: dict | None = None) -> int:
    """Write a job into the shared SQLite DB so the separate userbot.py process can pick it up."""
...
# ================= CANCEL =================
        ...
        if data.startswith("REG_CONFIRM:"):
            ...
            # 🔥 बाकी code SAME रहेगा (precredit, save_form_row, etc.)
            return
...
def _sqlite_checkpoint():
    """Flush WAL into main DB (safe even if WAL is not used)."""
    try:
        con = sqlite3.connect(DB, timeout=1, check_same_thread=False)
...
def _gdrive_download_to_db() -> bool:
    """Restore DB on boot (best-effort) from the exact DRIVE_FILE_ID."""
    svc = _gdrive_service()
    if not svc:
        return False
...
def _gdrive_upload_db() -> bool:
    """Upload/overwrite DB to the exact DRIVE_FILE_ID (fixed-id mode)."""
    svc = _gdrive_service()
    if not svc:
        return False
    if not DRIVE_FILE_ID:
